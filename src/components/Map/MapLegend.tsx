...
import { TYPE_COLORS, STATUS_COLORS, CATEGORY_COLORS } from './construction-map.constants';
import { getTypeLabel, getStatusLabelShort, getCategoryLabelShort } from './construction-map.utils';
...
export function MapLegend({
  ...
  const allStatusesSelected = visibleStatuses.size === Object.keys(STATUS_COLORS).length;
  const allCategoriesSelected = visibleCategories.size === Object.keys(CATEGORY_COLORS).length;

  return (
    <div className="absolute bottom-4 left-4 bg-card rounded-lg shadow p-3 border border-border max-w-[220px]">
      ...
      >
        {Object.entries(CATEGORY_COLORS).map(([category, color]) => (
          <FilterToggle
            key={category}
            color={color}
            label={getCategoryLabelShort(category)}
        ...
        >
          {Object.entries(STATUS_COLORS).map(([status, color]) => (
            <FilterToggle
              key={status}
              color={color}
              label={getStatusLabelShort(status)}
